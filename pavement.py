...
def _install_data_dir():
    ...
    try:
        ...
        with open(config) as f:
            ...
            xml = xml[:m.start(1)] + \
                "http://192.168.43.118:8080/geoserver" + xml[m.end(1):]
    ...
    try:
        ...
        with open(config) as f:
            ...
            xml = xml[:m.start(1)] + \
                "http://192.168.43.118:8000/o/token/" + xml[m.end(1):]
            m = re.search('userAuthorizationUri>([^<]+)', xml)
            xml = xml[:m.start(
                1)] + "http://192.168.43.118:8000/o/authorize/" + xml[m.end(1):]
            m = re.search('redirectUri>([^<]+)', xml)
            xml = xml[:m.start(
                1)] + "http://192.168.43.118:8080/geoserver/index.html" + xml[m.end(1):]
            m = re.search('checkTokenEndpointUrl>([^<]+)', xml)
            xml = xml[:m.start(
                1)] + "http://192.168.43.118:8000/api/o/v4/tokeninfo/" + xml[m.end(1):]
            m = re.search('logoutUri>([^<]+)', xml)
            xml = xml[:m.start(
                1)] + "http://192.168.43.118:8000/account/logout/" + xml[m.end(1):]
            with open(config, 'w') as f:
                f.write(xml)
    except Exception as e:
        print(e)
    ...
    try:
        ...
        with open(config) as f:
            ...
            xml = xml[:m.start(1)] + "http://192.168.43.118:8000" + xml[m.end(1):]
            with open(config, 'w') as f:
                f.write(xml)
    except Exception as e:
        print(e)
...
def start_geoserver(options):
    ...
    if urlparse(GEOSERVER_BASE_URL).hostname != '192.168.43.118':
        print "Warning: OGC_SERVER['default']['LOCATION'] hostname is not equal to '192.168.43.118'"
